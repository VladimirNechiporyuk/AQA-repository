package com.qa.lessons.database.impl;

import com.qa.lessons.data.ObjectWithData;
import com.qa.lessons.database.DataBase;
import com.qa.lessons.enums.DbCollectionNames;
...
public class DataBaseImpl implements DataBase {
    ...
    public ObjectWithData updateEntity(DbCollectionNames collectionName, UUID id, ObjectWithData value) {
        ObjectWithData object = data.get(collectionName).get(id);
        if (object == null) {
            System.out.printf("Value by key '%s' does not exists.%n", id);
        }
        data.get(collectionName).put(id, value);
        return data.get(collectionName).get(id);
    }
